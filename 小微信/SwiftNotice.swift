...
extension UIViewController {
    ...
    func noticeError(text: String, autoClear: Bool = false, autoClearTime: Int = 1) {
        SwiftNotice.showNoticeWithText(NoticeType.error, text: text, autoClear: autoClear, autoClearTime: autoClearTime)
    }
    func noticeInfo(text: String, autoClear: Bool = false, autoClearTime: Int = 1) {
...
class SwiftNotice: NSObject {
    ...
    static func showNoticeWithText(type: NoticeType,text: String, autoClear: Bool, autoClearTime: Int) {
        ...
        checkmarkView.frame = CGRectMake(30, 15, 36, 36)
        mainView.addSubview(checkmarkView)
        
        let label = UILabel(frame: CGRectMake(0, 60, 100, 16))
        label.font = UIFont.systemFontOfSize(13)
        label.textColor = UIColor.whiteColor()
        label.text = text
        label.textAlignment = NSTextAlignment.Center
